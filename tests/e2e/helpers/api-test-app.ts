...
/**
 * Thin orchestrator for E2E test suite lifecycle.
 *
 ...
 *
 *   app.requester.sendGraphQL(...)     // authenticated
 *   app.withoutAuth().sendGraphQL(...) // no credentials
 */
export class ApiTestApp {
  ...
  async bootstrap({
    ...
    const restAuthHeaders: Record<string, string> = {};
    if (restApi === 'publicApi') {
      if (!restApiConfig.publicApiKey) {
        throw new Error('PUBLIC_API_KEY is required for public-api E2E (IAM + x-api-key)');
      }
      restAuthHeaders['x-api-key'] = restApiConfig.publicApiKey;
    } else if (restApi === 'authApi') {
      restAuthHeaders.Cookie = `project_access_token=${idToken}`;
      if (!restApiConfig.authApiKey) {
        throw new Error('AUTH_API_KEY is required for auth-api E2E (IAM + x-api-key)');
      }
      restAuthHeaders['x-api-key'] = restApiConfig.authApiKey;
    }
    ...
    this.requester = new Requester(
      ...
      new RestApi({
        ...
        // public-api uses SigV4 (AWS_IAM) + x-api-key; auth-api uses cookie (+ optional x-api-key)
        credentials: restApi === 'publicApi' ? credentials : undefined,
        authHeaders: Object.keys(restAuthHeaders).length > 0 ? restAuthHeaders : undefined,
      }),
      this.detector,
    );
  ...
  getAuthenticatedCognitoSub(): string {
    if (!this._idToken) {
      throw new Error('ApiTestApp.bootstrap() must be called before getAuthenticatedCognitoSub()');
    }
...
let instance: ApiTestApp | undefined;

export function getApiTestApp(): ApiTestApp {
  if (!instance) {
    instance = new ApiTestApp();
  }
  return instance;
}
