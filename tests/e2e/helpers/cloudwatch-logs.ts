...
export class LogCollector implements ILogCollector {
  ...
  async terminate(): Promise<void> {
    ...
    // console.info([`Waiting ${waitTime}ms for logs to flush before stopping live tail and asserting...`]);
    await sleep(waitTime);
    this.stopFn?.();
  }

  /**
   * Starts a CloudWatch Logs Live Tail session covering all given log group ARNs.
   * Received log events are routed into this collector keyed by their log group identifier.
   * Returns a stop function — called in terminate().
   */
  private async startLiveTail(logGroupIdentifiers: string[]): Promise<() => void> {
    ...
    console.log(`Started CloudWatch Logs Live Tail session for groups:`, logGroupIdentifiers);
    ...
    const consumeStream = async () => {
      try {
        for await (const event of stream) {
          // This specific block is guaranteed by AWS to arrive first
          if (event.sessionStart) {
            console.log(`Stream is ready to receive log events for groups:`, logGroupIdentifiers);
            signalStreamReady();
          }
          ...
          if (event.sessionUpdate) {
            ...
            for (const log of logs) {
              ...
              // console.dir([`Received log event from [${group}] logs:`, line], {depth: null});
              if (line) this.push(group, line);
            }
          }
        }
      } catch (error) {
        console.log(['Stream encountered an error:', error]);
      }
    };
    ...
    return () => {
      ...
      client.destroy();
      console.log(`Stopped CloudWatch Logs Live Tail session for groups:`, logGroupIdentifiers);
    };
  }
}
