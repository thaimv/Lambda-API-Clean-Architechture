...
describe('NotificationService + SnsNotifier + SnsEndpointManager', () => {
  let notificationService: NotificationService;

  beforeEach(() => {
    vi.clearAllMocks();
    notificationService = new NotificationService(createMockAppConfig());
  });
  ...
  it('should return SnsNotifier instance from getNotifier()', () => {
    const notifier = notificationService.getNotifier(NOTIFICATION_PROVIDER.SNS);
    expect(notifier).toBeInstanceOf(SnsNotifier);
  });

  it('should return SnsEndpointManager instance from getEndpointManager()', () => {
    const endpointMgr = notificationService.getEndpointManager(NOTIFICATION_PROVIDER.SNS);
    expect(endpointMgr).toBeInstanceOf(SnsEndpointManager);
  });
  ...
  describe('SnsNotifier', () => {
    let notifier: SnsNotifier;

    beforeEach(() => {
      notifier = notificationService.getNotifier(NOTIFICATION_PROVIDER.SNS) as SnsNotifier;
    });
  ...
  describe('SnsEndpointManager', () => {
    let endpointMgr: SnsEndpointManager;

    beforeEach(() => {
      endpointMgr = notificationService.getEndpointManager(
        NOTIFICATION_PROVIDER.SNS,
      ) as SnsEndpointManager;
    });
