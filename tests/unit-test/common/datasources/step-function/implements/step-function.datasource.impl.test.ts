...
describe('StepFunctionDatasource Provider', () => {
  ...
  beforeEach(() => {
    ...
    stepFunctionProvider = new StepFunctionDatasource(createMockAppConfig(), mockSFNClient);
    vi.clearAllMocks();
  });
  ...
  describe('constructor', () => {
    test('should create StepFunctionDatasource instance with provided client', () => {
      const customClient = new SFNClient({ region: 'us-west-2' });
      const stepFunction = new StepFunctionDatasource(createMockAppConfig(), customClient);

      expect(stepFunction).toBeInstanceOf(StepFunctionDatasource);
    });

    test('should create StepFunctionDatasource instance with default client when no client provided', () => {
