...
describe('CognitoIdentityDatasource', () => {
  ...
  describe('getOpenIdToken', () => {
    it('should return identity id and token from cognito response', async () => {
      mockSend.mockResolvedValue({
        IdentityId: 'identity-123',
        Token: 'openid-token',
      });

      const result = await datasource.getOpenIdToken('user-sub');

      expect(GetOpenIdTokenForDeveloperIdentityCommand).toHaveBeenCalled();
      expect(mockSend).toHaveBeenCalledTimes(1);
    ...
    it('should default missing fields to empty strings', async () => {
      mockSend.mockResolvedValue({});
  ...
  describe('getIdentityForUserPoolToken', () => {
    it('should return identity id and pass through the user pool id token', async () => {
      const idToken = 'header.payload.signature';
      mockSend.mockResolvedValue({ IdentityId: 'ap-northeast-1:identity-abc' });

      const result = await datasource.getIdentityForUserPoolToken(idToken);

      expect(GetIdCommand).toHaveBeenCalledWith({
        IdentityPoolId: 'test-identity-pool',
        Logins: {
          'cognito-idp.ap-northeast-1.amazonaws.com/ap-northeast-1_testpool': idToken,
        },
      });
      expect(result).toEqual({
        identityId: 'ap-northeast-1:identity-abc',
        token: idToken,
      });
    });
    ...
    it('should default missing identity id to empty string', async () => {
      ...
      const result = await datasource.getIdentityForUserPoolToken('token');
