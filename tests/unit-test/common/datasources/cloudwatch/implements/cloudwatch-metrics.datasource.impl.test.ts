...
describe('CloudWatch Metrics Provider', () => {
  ...
  beforeEach(() => {
    ...
    cloudWatchMetrics = new CloudWatchMetricsDatasource(
      createMockAppConfig(),
  ...
  describe('constructor', () => {
    test('should create CloudWatchMetricsDatasource instance with provided client', () => {
      const customClient = new CloudWatchClient({ region: 'us-west-2' });
      const metrics = new CloudWatchMetricsDatasource(createMockAppConfig(), customClient);

      expect(metrics).toBeInstanceOf(CloudWatchMetricsDatasource);
    });

    test('should create CloudWatchMetricsDatasource instance with default client when no client provided', () => {
