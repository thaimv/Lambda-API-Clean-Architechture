...
vi.mock('@/common/lambda/pipes/api-logging.pipe', () => ({
  ApiLoggingPipe: vi.fn().mockImplementation(() => ({
    handle: vi.fn(async (passable, next) => next(passable)),
  })),
}));

vi.mock('@/common/lambda/pipes/api-error-handling.pipe', () => ({
  ApiErrorHandlingPipe: vi.fn().mockImplementation(() => ({
    handle: vi.fn(async (passable, next) => next(passable)),
  })),
}));
