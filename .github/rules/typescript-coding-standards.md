...
| Classes                    | PascalCase                                   | `UserUseCase`                               |
...
| Module DI tokens           | `I`-prefixed key in `{MODULE}_DI_CONST`      | `USER_DI_CONST.IUserUseCase`                |
...
| `{lambda-name}.ts` | Lambda entry point (presenter), e.g. `public-api.ts`             |
...
export class UserRepo implements IUserRepo {
  constructor(
    ...
    @inject(DI.DB_CLIENT_DATASOURCE) private readonly dbClient: PrismaDBClientDatasource,
  ) {}
}
```
...
- Use the `@/` path alias (maps to `src/`), e.g. `@/common/...`, `@/modules/user/...`.
...
```typescript
// src/modules/user/user.const.ts
export const USER_DI_CONST = {
