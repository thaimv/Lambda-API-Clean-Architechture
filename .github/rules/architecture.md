...
- `public-api.module.ts` and `appsync-api.module.ts` are `@Module`s that `imports` the datasource DI modules + the relevant domain module (`UserRestModule` / `UserGraphModule`).
...
- Module-specific tokens → `{module}.const.ts` (e.g. `USER_DI_CONST.IUserUseCase`).
...
@Module({
  ...
  providers: [
    { provide: USER_DI_CONST.IUserRepo, useClass: UserRepo },
    { provide: USER_DI_CONST.IUserUseCase, useClass: UserUseCase },
  ],
})
...
export class UserUseCase implements IUserUseCase {
  constructor(@inject(USER_DI_CONST.IUserRepo) private readonly userRepo: IUserRepo) {}
}
```
...
```typescript
// src/presenter/lambdas/public-api.ts
