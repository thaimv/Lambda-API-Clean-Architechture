...
Base/template backend for AWS Lambda APIs. Node.js · TypeScript · AWS Lambda · Prisma (Aurora PostgreSQL) · DynamoDB (scaffolding) · REST + GraphQL (AppSync) · Clean Architecture. Ships infrastructure + one example `users` module.
...
│   ├── lambda-spec-review/      # Spec review workflow
...
**Flow**: skill invoked → reads brain → loads relevant rules → executes workflow → produces output.
...
| Spec review workflow                           | `skills/lambda-spec-review/`           |
...
- When reviewing/developing a module, load its `brain/modules/{ModuleName}.md` for context. `UsersModule.md` is the reference example to copy.
...
- Lambda entry points: `src/presenter/lambdas/public-rest-api.ts` and `appsync-api.ts`. Composition modules in `src/modules/*.module.ts`.
