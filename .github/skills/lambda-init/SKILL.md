...
**`/lambda-init`**: Scan codebase → update `modules`, `lambdas`, `repo_layout`, and `commands` in `project-context.json`.

**`/lambda-init --check`**: Compare codebase vs `project-context.json` → report drift and MCP status. Does **not** rewrite module/lambda maps. If context is missing, stop and ask to run `/lambda-init` first.
