...
3. If `prev_skill` is a list, the current run must follow **one of** those skills (e.g. `lambda-graphql-impl` or `lambda-rest-impl`).
...
3. Report summary to the user (success, gaps, or next steps — suggest `next_skill` when applicable; if `next_skill` is a list, suggest the path that matches the API style just implemented).
