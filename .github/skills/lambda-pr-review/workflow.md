...
2. **Fetch diff locally via git** (preferred):
   ```bash
   ...
   ```
3. **Validate content**: If filtered diff is empty (no reviewable files) → stop:
   ```
...
| Check           | Pass when                          |
| --------------- | ---------------------------------- |
| Review report   | Delivered in chat                  |
