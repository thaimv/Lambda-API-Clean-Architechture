...
- When a user submits their profile, the API must upsert the record into the `users` table keyed by `cognitoSub`.

❌ Do not combine multiple unrelated features in a single PBI.
✅ Explain domain terms when they are not common (e.g., "cognitoSub is the Cognito user sub (UUID) carried in the Cognito identity context").
...
- `UserUseCase.saveUser` orchestrates → `UserRepo.saveUser` performs a Prisma upsert keyed by `cognitoSub`.
