...
description: 'Verify API implementation matches the PBI spec — compare source code against documented requirements. Use when: user asks to check spec compliance, verify implementation against spec, or validate code matches PBI.'
argument-hint: 'Path to PBI spec file (e.g. document/specs/pbi-123.md); optional module name when spec lists multiple'
---
...
/lambda-spec-review {path-to-spec.md} {module-name}
```

Second argument optional — use when the spec affects multiple modules and you want to scope the review to one folder under `src/modules/`.

## Goal

Read the PBI spec → read the implemented module source (and detail design when present) →
map each spec requirement to code → report **compliance gaps** where implementation diverges from or omits spec behavior.

This is a **post-implementation** check. Run after `lambda-graphql-impl` or `lambda-rest-impl` (see `manifest.yaml` → `prev_skill`), before generating tests.
