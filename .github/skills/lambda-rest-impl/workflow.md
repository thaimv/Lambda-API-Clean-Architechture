...
export class MyModuleRestController {
  ...
  @Get(PathPublicRestApi.GetMyResource)
  ...
  @Post(PathPublicRestApi.CreateMyResource)
...
```ts
// src/modules/public-rest-api.module.ts
@Module({
  imports: [
...
## Step 9: Unit tests (`tests/unit-test/modules/[module]/`)

Mirror the `src/` tree:

```
tests/unit-test/modules/[module]/
├── controllers/[module].rest.controller.test.ts
...
- ❌ Missing required path/body param → Zod parse error
- ❌ Invalid type / out-of-range → Zod parse error
- ❌ Resource not found → `NotFoundError`
- ❌ Repository throws unexpected error → `InternalServerError`
...
Errors are handled centrally by `ApiErrorHandlingPipe`. Throw the appropriate class — do NOT manually build error JSON.
...
Lambda.createHandler()              [src/common/lambda/lambda.ts]
     │  ApiLoggingPipe → ApiErrorHandlingPipe → RestApiRouter.invoke()
     ▼
