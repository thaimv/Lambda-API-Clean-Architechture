...
- `UsersUseCase.saveUser(dto, authUser)` → `UsersRepo.saveUser` performs Prisma `upsert` on `users` table keyed by `gigya_uuid`.
...
participant DB as PostgreSQL
    ...
    Lambda->>UsersRepo: findActiveUserByNickname(userNickname)
    UsersRepo->>DB: SELECT users WHERE user_nickname = ? AND delete_datetime IS NULL
    DB-->>UsersRepo: row or null
    Lambda-->>AppSync: EB-009 if nickname belongs to another user

    Lambda->>UsersRepo: saveUser(dto, authUser)
    UsersRepo->>DB: UPSERT users WHERE gigya_uuid = gigyaUuid
    DB-->>UsersRepo: upserted row
    UsersRepo-->>Lambda: { gigyaUuid, userNickname, cognitoId }
