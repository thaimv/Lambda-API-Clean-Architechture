...
Wired through domain modules — not imported directly in `AppsyncApiModule` / `PublicApiModule`:
...
| `repos/user.di` (`CommonUserRepo`) | `DI.COMMON_USER_REPO`     | `AppsyncApiModule`, `PublicApiModule` |

## Composition modules (direct imports)

| Module                  | Imports                                          |
| ----------------------- | ------------------------------------------------ |
| `appsync-api.module.ts` | `CommonUserRepo`, `UserGraphModule`, `AppConfig` |
| `public-api.module.ts`  | `CommonUserRepo`, `UserRestModule`, `AppConfig`  |
