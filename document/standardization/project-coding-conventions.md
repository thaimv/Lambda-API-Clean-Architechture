...
| Classes            | PascalCase                   | `UserUseCase`, `UserGraphController`        |
| Interfaces         | PascalCase with `I` prefix   | `IUserRepo`, `IUserUseCase`                 |
...
| DI tokens (module) | `{MODULE}_DI_CONST.I{Name}`  | `USER_DI_CONST.IUserUseCase`                |
...
REST paths are defined in `src/common/constants/rest-api.const.ts` (`PathPublicApi`) and mapped via `@Get` / `@Post` decorators.
...
- Every PR runs automated tests via GitHub Actions (`.github/workflows/`).
