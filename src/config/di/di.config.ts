...
const bindProvider = (provider: TProviderMetadata) => {
 ...
 */
export function bootstrapApplication(entryModule: TNewable<unknown>, router: IRouter<unknown>) {
  const { imports, providers, controllers } = getModuleMetadata(entryModule);

  providers?.forEach(bindProvider);

  // Bind and add controllers to the router
  ...
  if (imports) {
    imports.forEach((importModule) => {
      bootstrapApplication(importModule, router);
    });
  }
}
