...
export const GIGYA_ID_PATTERN = /[a-f0-9]{32}/gi;
...
export const jsonReplacerFn = (key: string, value: unknown) => {
  ...
  // Check if this field contains gigyaId in its value (like file paths)
  if (
    typeof value === 'string' &&
    typeof snakeCaseKey === 'string' &&
    CONTAINING_SENSITIVE_FIELDS.includes(snakeCaseKey)
  ) {
    return value.replace(GIGYA_ID_PATTERN, (match: string) => {
